...
def show_reads(r,inds, fig = None, subplot = 111, flagged = None):
    ...
    seismic.seismic( r.reads[inds].T, ax = ax, colors = ct,continuous = False,
                    labels = r.read_idx_dict.keys(),yunits = 'reads'
                    ,xunits = 'position from '+str(inds[0]))
    
    refarray = r.refarray
    seismic.seismic( r.reads[inds].T * refarray[inds].T,
                    ax = ax, colors = 'w'*ni,continuous = False,
                    label_x = False, label_y = False)


    if flagged != None:
        seismic.seismic( r.reads[inds].T * flagged,
...
def view_motifs(r,motifs,showidx=-1):

    m_errors = []

    for k,m in motifs.iteritems():
        if not m: continue
        s = array(m['starts'],int)
        l = array(m['lens'],int)
        f = s+l 
        n = len(s)

        l0 = l[0]
        #are motif lengths always the same?
        total_reads = zeros((n,l0))
        total_errors = zeros((n,l0))

        for i in range(n):
            inds = arange(s[i], f[i])
            mr = r.reads[inds,:]
            mref = r.refarray[inds,:]
        
            total_reads[i,:] = np.sum(mr,1)
            total_errors[i,:] = np.sum(mr *(1-mref),1)
            
            
                
        total_reads_summed = np.sum(total_reads,0)
        total_errors_summed = np.sum(total_errors,0)

        errfrac = total_errors/total_reads
        edict = {}
        edict['reads'] = total_reads
        edict['errors'] = total_errors
        edict['total_reads_s'] = total_reads_summed
        edict['total_errors_s'] = total_errors_summed
        edict['errfrac'] = errfrac

        m_errors.append([edict, k])

    mean_fracs = (map(lambda e: np.mean(e[0]['errfrac'],0), m_errors))
    max_inds = (map(lambda e: np.argmax(np.sum(e[0]['errfrac'],1)), m_errors))
    me = [ m_errors[i][0]['errfrac'][max_inds[i],:] for i in range(len(m_errors)) ]
    print me
    max_errs = me

    mnames = map(lambda e: e[1], m_errors)
    srt = argsort(map(lambda x : np.mean(x),mean_fracs))[::-1]

    count = 0
    for idx in srt:
        print '{0:3} - {1:20}{2:20}{3:>5}%   max:{4:>5}%   n={5:<5}'.format( count,mnames[idx], 'mean error rate: ',  str(round(mean(mean_fracs[idx])*100,2)),str(round(mean(max_errs[idx])*100)),len(m_errors[idx][0]['errfrac']))
        if showidx == count:
            showi = max_inds[idx]
            mname = m_errors[idx][1]
            m_instance = motifs[mname]
            s = m_instance['starts'][showi]
            l = m_instance['lens'][showi]
            mseq = m_instance['seqs'][showi]
            margin = 10
            show_zoom(r,inds = [s - margin*l, s+l + margin*l], motif = mseq)
            f = plt.figure(1)
            f.clear()
            ax = f.add_subplot(111)
            show_motif_errors(r, m_instance['starts'], m_instance['lens'][0],margin = margin,ax = ax)
            myplots.maketitle(ax,'Error rate vs motif instance')
        count += 1
        

    #print r.error
def show_motif_errors(r, m_starts, m_len, ax = None, margin = 8):

    winsize = m_len + m_len*margin*2
    win_hash_t = zeros(winsize)
    win_hash_e = zeros(winsize)
    n = len(m_starts)
    win_t = zeros((winsize,n))
    win_e = zeros((winsize,n))

    for i in range(n):
        s = m_starts[i]
        i0 = s - m_len*margin
        i1 = s + m_len + m_len*margin
        ref = r.refarray[i0:i1,:]
        reads = r.reads[i0:i1,:]

        win_t[:,i] = np.sum(reads,1)
        win_e[:,i] = np.sum(reads*(1-ref),1)

        win_hash_t += np.sum(reads,1)
        win_hash_e += np.sum(reads * (1-ref),1)

    #seismic.seismic([win_hash_e/win_hash_t],ax=ax)
    where_m = zeros(winsize)
    where_m[m_len*margin:m_len*margin + m_len] = 1

    ct = mycolors.getct(n)
    print n

    seismic.seismic((win_e).T,xax = arange(winsize),ax=ax,
                    xunits = 'seq position',
                    yunits = 'error freq',stacked = True, continuous = False,
                    colors = ct, labelinds = [0])
   #seismic.seismic([win_hash_e/win_hash_t * where_m],ax=ax, colors = ['orange'],
   #                 label_x = False, label_y = False,stacked = True)

def show_zoom(r,matches = True, inds = None, motif = 'GATGAG'):
    ...
    if inds == None:
        ...
        i1 = inds[1]
        mm_inds = inds_at_which(r, 'mismatch')
        mmi0 = np.max(np.nonzero(less(mm_inds,i0))[0])
        mmi1 = np.min(np.nonzero(greater(mm_inds,i1))[0])
    ...
    myplots.maketitle(ax2  ,'', 'Same region, yeast motif ' + motif + ' highlighted in orange')
