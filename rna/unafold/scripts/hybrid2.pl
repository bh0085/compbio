...
my $same;
open IN, '<', $file1 or open IN, '<', "$file1.seq" or die "Can't open $file1: $!";
$seq1 = join '', <IN>;
close IN or die $!;
...
if (defined $file2 and $file2 ne $file1) {
    ($prefix2 = $file2) =~ s/\.seq$//;
    $prefix2 =~ s/^.+[\/\\]//;
    open IN, '<'. $file2 or open IN, '<', "$file2.seq" or die "Can't open $file2: $!";
    $seq2 = join '', <IN>;
    close IN or die $!;
...
if ($same) {
	...
	unless ($exclA) {
	    ...
	    close IN or die $!;
	    safelyOpen *OUT, '>', "$prefix1.inf";
	    print OUT $ZaInf, "\n";
	    close OUT or die $!;
	}
	unless ($exclAA) {
	    my $ZaaInf;
	    safelyOpen *IN, '<', "$prefix1-$prefix1.dG";
	    scalar <IN>;
	    while (<IN>) {
		my ($t, $g, $z) = split;
	    ...
	    close IN or die $!;
	    safelyOpen *OUT, '>', "$prefix1-$prefix1.inf";
	    print OUT $ZaaInf, "\n";
	    close OUT or die $!;
	}
    }
...
if ($options{infinity}) {
    ...
    unless ($exclBB) {
	my $ZbbInf;
	safelyOpen *IN, '<', "$prefix2-$prefix2.dG";
	scalar <IN>;
	while (<IN>) {
	    my ($t, $g, $z) = split;
	...
	close IN or die $!;
 	safelyOpen *OUT, '>', "$prefix2-$prefix2.inf";
	print OUT $ZbbInf, "\n";
	close OUT or die $!;
   }
    my $ZabInf;
    safelyOpen *IN, '<', "$prefix1-$prefix2.dG";
    scalar <IN>;
    while (<IN>) {
	my ($t, $g, $z) = split;
    ...
    close IN or die $!;
    safelyOpen *OUT, '>', "$prefix1-$prefix2.inf";
    print OUT "$ZabInf\n";
    close OUT or die $!;
}
...
sub plot {
    ...
    my $TmConc;
    safelyOpen *TMCONC, '<', "$prefix.TmConc";
    while (<TMCONC>) {
	($TmConc) = split;
    }
    close TMCONC or die $!;
    ...
    safelyOpen *TMCP, '<', "$prefix.ens.TmCp";
    while (<TMCP>) {
	($TmCp, $Cp) = split;
    }
    close TMCP or die $!;
    if (-s "$prefix.obs.Tm") {
	safelyOpen *TM, '<', "$prefix.obs.Tm";
	($TmObs, $Obs) = split /\t/, <TM>;
	close TM or die $!;
    }
	...
	safelyOpen *TMEXT, '<', "$prefix.ens.TmExt2";
	($TmExt, $Ext) = split /\t/, <TMEXT>;
	close TMEXT or die $!;
	undef $TmExt if defined $TmExt and $TmExt eq 'nan';
	safelyOpen *MAXEXT, '<', "$prefix.ens.MaxExt";
	chomp ($maxExt = <MAXEXT>);
	close MAXEXT or die $!;
    }
