...
dnl found, the output variables GD_CFLAGS, GD_LDFLAGS and GD_LIBS will contain
dnl the compiler flags, linker flags and libraries necessary to use gd;
dnl otherwise, those variables will be empty.  In addition, the symbol
dnl HAVE_GD is defined if the library is found, and the symbols HAVE_GD_GIF,
dnl HAVE_GD_JPEG and HAVE_GD_PNG are defined if the lirbary supports creating
dnl images in gif, jpeg and png formats, respectively.
dnl
dnl The user may use --with-gd=no or --without-gd to skip checking for the
dnl library.  (The default is --with-gd=yes.)  If the library is installed in
dnl an unusual location, --with-gd=DIR will cause the macro to look for
dnl gdlib-config in DIR/bin or, failing that, for the headers and libraries in
...
if test "X$with_apple_opengl_framework" = "Xyes"; then
  ...
  GLUT_LIBS="-lobjc -framework GLUT ${GL_LIBS}"
else
