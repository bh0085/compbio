function binary_regression4(input)
...
[gene_tot,tf_tot,net_prob,gene_names,tf_names]=prepare_data4(filename,control);
...
%     plot_profile(gene_sq,tf_sq,pf,5);
    ...
    pf=adjusted_profile_thresholding2(pf,max_num3,tf_sq,gene_sq);
...
%     [pf{:,2}]
    ...
    if iter_num~=0
        for iter=1:iter_num
    iter;        
            pf=profile_update2(pf,gene_sq,tf_sq,max_num3,f);
...
%         disp('no infered profile')
        ...
        pf_last=profile_sorting(pf,g_pattern);
        pf_last_not=profile_sorting(pf,my_not(g_pattern));
    end
    ...
    pf_circuit= remove_similar_circuits(pf_circuit,tf_sq);
    pf_circuit2=profile_thresholding(pf_circuit,th_last);
...
%   if no model, choose the best existent one  
    ...
    elseif length([pf_circuit{:,3}])==0
        ...
        pf_circuit=profile_thresholding(pf,th_last);        
    elseif length([pf_circuit{:,3}])==0
        
        pf_circuit=profile_thresholding(pf,th_last/2);
    end
...
save(results_filename,'network_cell','infered_network','quality_test')
