...
def run0(meth = 'moment', 
    ...
    for i in range(itr):
        ...
        if meth == 'moment':
            s,srto = get_moment_sort(s)
        if meth == 'maxfirst':
            s,srto = get_maxfirst(s)
        elif meth == 'rnd':
            this_dlt = []
            s = rnd_shuffles(s,
...
def get_moment_sort(arr):
    ...
    moments = np.argmax(arr, 0) #np.sum(yvals[:,:] * arr**8,0)/np.sum(arr**8,0)
    srt_red = argsort(moments[:])[::-1]

    return arr[:,srt_red], srt_red
...
def get_maxfirst(arr):
    ...
    for i in range(len(shuf_tgs)):
        out[(shuf_tgs[i][1])] = arr[(shuf_tgs[i][0])]
