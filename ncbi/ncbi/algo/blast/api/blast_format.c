...
/** Deletes matrix generated by s_LoadMatrix
 * @param matrix data to be freed [in]
 */
const int k_MatrixSize = 128;
static void s_DeleteMatrix(Int4** matrix)
{
    int i;
    if (matrix == NULL)
      return;

    for(i=0; i<k_MatrixSize; i++)
         MemFree(matrix[i]);
...
/** Adopted from api/jzcoll.c:load_default_matrix
 * We take the compressed matrix and move it 
 * to a format that includes all ASCII letters,
 * though most cells will be blank.  this is understodd
 * by txalign.c and does not seem worth fixing now.
 * @param name matrix name (e.g., BLOSUM90) [in]
 */
static Int4** s_LoadMatrix(const char* name)
{
    Int4** ss;
    int i, j;
    int dim = 0;
    ...
    pchars = pack_matrix->symbols;
    dim = strlen(pchars);

    ss = (Int4**) MemNew((size_t)k_MatrixSize * sizeof (Int4*));
    if (ss == NULL)
       return NULL;

    for(i = 0; i<k_MatrixSize; ++i)
           ss[i] = (Int4*) MemNew((size_t)k_MatrixSize * sizeof (Int4));

    for(i = 0; i < k_MatrixSize; i++)
           for(j = 0; j < k_MatrixSize;j++)
                 ss[i][j] = -1000;

    for(i = 0; i < dim; ++i)
           for(j = 0; j < dim; ++j)
                 ss[pchars[i]][pchars[j]] = pack_matrix->scores[(i*dim)+j];
