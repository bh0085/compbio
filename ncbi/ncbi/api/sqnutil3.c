...
static Boolean InWordBeforeCytochromeOrCoenzyme (CharPtr cp, CharPtr start)
{
  ...
  if (cp - start >= 9 && StringNCmp (cp - 9, "cytochrome", 10) == 0) {
    return TRUE;
...
static SuspectProductNameData suspect_product_terms[] = {
  ...
  { "putative enzyme", IsSingleWordOrWeaselPlusSingleWord },
  { "enzyme", IsSingleWordOrWeaselPlusSingleWord },
...
NLM_EXTERN void FindCommentDescriptors (ValNodePtr PNTR discrepancy_list, ValNodePtr sep_list)
{
  ...
  if (desc_list != NULL) {
    ValNodeAddPointer (discrepancy_list, 0, 
                       NewClickableItem (ONCALLER_COMMENT_PRESENT, "%d comment descriptors were found", desc_list));
  }
}
...
static Boolean ExcludeFromFailureText (Int4 i)
{
  if (i == eBarcodeTest_PercentN || i == eBarcodeTest_Length)
  {
    return TRUE;
  } 
  else 
...
static CharPtr FailureTextFromBarcodeTestResults (BarcodeTestResultsPtr res)
{
  ...
  for (i = 0; i < eBarcodeTest_LAST; i++)
  {
    if (ExcludeFromFailureText(i))
    {
      /* excluded from failure report */
      continue;
    }
    if (res->failed_tests[i]) 
    {
