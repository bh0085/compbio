...
NLM_EXTERN Boolean LIBCALL MolInfoAsnWrite (MolInfoPtr mip, AsnIoPtr aip, AsnTypePtr orig)
{
    ...
    if (mip->gbmoltype != NULL)
    {
        av.ptrvalue = (Pointer)mip->gbmoltype;
        if (! AsnWrite(aip, MOLINFO_gbmoltype, &av)) goto erret;
    }
...
NLM_EXTERN MolInfoPtr LIBCALL MolInfoAsnRead (AsnIoPtr aip, AsnTypePtr orig)
{
    ...
    while ((atp = AsnReadId(aip, amp, atp)) != oldatp)
    {
        ...
        else if (atp == MOLINFO_gbmoltype)
            mip->gbmoltype = (CharPtr)av.ptrvalue;
    }
    if (AsnReadVal(aip, atp, &av) <= 0) goto erret;    /* end struct */
ret:
    AsnUnlinkType(orig);       /* unlink local tree */
...
*
*****************************************************************************/
NLM_EXTERN TextAnnotIdPtr LIBCALL TextAnnotIdNew (void)
{
    TextAnnotIdPtr taip;

    taip = (TextAnnotIdPtr)MemNew(sizeof(TextAnnotId));
    if (taip == NULL) return taip;

    taip->version = INT2_MIN;
    return taip;
}
...
*
*   TextAnnotIdFree(anp)
*       Frees one TextAnnotId
*
*****************************************************************************/
NLM_EXTERN TextAnnotIdPtr LIBCALL TextAnnotIdFree (TextAnnotIdPtr taip)
{
    if (taip == NULL)
        return taip;
    
    MemFree(taip->name);
    MemFree(taip->accession);
    MemFree(taip->release);
    return (TextAnnotIdPtr)MemFree(taip);
}

/*****************************************************************************
*
*   TextAnnotIdAsnWrite(taip, aip, atp)
*       atp is the current type (if identifier of a parent struct)
*       if atp == NULL, then assumes it stands alone (TextAnnotId ::=)
*
*****************************************************************************/
NLM_EXTERN Boolean LIBCALL TextAnnotIdAsnWrite (TextAnnotIdPtr taip, AsnIoPtr aip, AsnTypePtr orig)
{
    DataVal av;
    AsnTypePtr atp;
    Boolean retval = FALSE;
    ...
    atp = AsnLinkType(orig, TEXTANNOT_ID);   /* link local tree */
    if (atp == NULL)
        return retval;
    ...
    if (taip->name != NULL)
    {
        av.ptrvalue = taip->name;
        if (! AsnWrite(aip, TEXTANNOT_ID_name, &av))
            goto erret;
    }
    if (taip->accession != NULL)
    {
        av.ptrvalue = taip->accession;
        if (! AsnWrite(aip, TEXTANNOT_ID_accession, &av))
            goto erret;
    }
    if (taip->release != NULL)
    {
        av.ptrvalue = taip->release;
        if (! AsnWrite(aip, TEXTANNOT_ID_release, &av))
            goto erret;
    }
    if (taip->version != INT2_MIN)
    {
        av.intvalue = (Int4) taip->version;
        if (! AsnWrite(aip, TEXTANNOT_ID_version, &av))
            goto erret;
    }
    if (! AsnCloseStruct(aip, atp, (Pointer)taip))
        goto erret;
    retval = TRUE;
erret:
    AsnUnlinkType(orig);       /* unlink local tree */
...
*
*****************************************************************************/
NLM_EXTERN TextAnnotIdPtr LIBCALL TextAnnotIdAsnRead (AsnIoPtr aip, AsnTypePtr orig)
{
    DataVal av;
    AsnTypePtr atp, oldtype;
    TextAnnotIdPtr taip=NULL;
    ...
    if (orig == NULL)           /* TextAnnotId ::= (self contained) */
        atp = AsnReadId(aip, amp, TEXTANNOT_ID);
    else
        atp = AsnLinkType(orig, TEXTANNOT_ID);    /* link in local tree */
    oldtype = atp;
    if (atp == NULL)
        return taip;
    ...
    while ((atp = AsnReadId(aip, amp, atp)) != oldtype)
    {
        ...
        if ((AsnReadVal(aip, atp, &av)) <= 0)
            goto erret;
        if (atp == TEXTANNOT_ID_name)
            taip->name = (CharPtr)av.ptrvalue;
        else if (atp == TEXTANNOT_ID_accession)
            taip->accession = (CharPtr)av.ptrvalue;
        else if (atp == TEXTANNOT_ID_release)
            taip->release = (CharPtr)av.ptrvalue;
        else if (atp == TEXTANNOT_ID_version)
            taip->version = (Int2)av.intvalue;
        else
            goto erret;
    }
    if ((AsnReadVal(aip, atp, &av)) <= 0)      /* read END_STRUCT */
...
NLM_EXTERN AnnotIdPtr LIBCALL AnnotIdFree (AnnotIdPtr anp)
{
    ...
    switch (anp->choice)
    {
        ...
        case ANNOTID_GENERAL:      /* general */
            DbtagFree((DbtagPtr)pnt);
            break;
        case ANNOTID_OTHER:      /* other */
            TextAnnotIdFree((TextAnnotIdPtr)pnt);
            break;
    }
...
NLM_EXTERN Boolean LIBCALL AnnotIdAsnWrite (AnnotIdPtr anp, AsnIoPtr aip, AsnTypePtr orig)
{
    ...
    switch (anp->choice)
    {
        ...
        if (aip->spec_version == 3 )    /* ASN3 strip new value */
        {
            ErrPostEx(SEV_ERROR,0,0,"ASN3: AnnotIdOther stripped");
        }
        else
            retval = TextAnnotIdAsnWrite((TextAnnotIdPtr)(anp->data.ptrvalue), aip, ANNOT_ID_other);
            break;
    }
erret:
    AsnUnlinkType(orig);       /* unlink local tree */
...
NLM_EXTERN AnnotIdPtr LIBCALL AnnotIdAsnRead (AsnIoPtr aip, AsnTypePtr orig)
{
    ...
    else if (atp == ANNOT_ID_other)
    {
        anp->choice = ANNOTID_OTHER;
        anp->data.ptrvalue = (Pointer)TextAnnotIdAsnRead(aip, atp);
        if (anp->data.ptrvalue == NULL) goto erret;
    }
ret:
    AsnUnlinkType(orig);       /* unlink local tree */
...
NLM_EXTERN AnnotIdPtr LIBCALL AnnotIdSetAsnRead (AsnIoPtr aip, AsnTypePtr settype, AsnTypePtr elementtype)
{
    ...
    while ((atp = AsnReadId(aip, amp, atp)) != settype)
    {
        ...
        else if ((aip->spec_version == 3 ) &&   /* ASN3 strip new value */
            (anp->choice == ANNOTID_OTHER))
        {
            ErrPostEx(SEV_ERROR,0,0,"ASN3: AnnotIdOther stripped");
            AnnotIdFree(anp);
