...
*    Generated using ASNCODE Revision: 6.17 at May 6, 2010  5:13 PM
*
**************************************************/
...
GBXrefAsnRead(AsnIoPtr aip, AsnTypePtr orig)
{
   ...
   if (atp == GBXREF_db_url) {
      if ( AsnReadVal(aip, atp, &av) <= 0) {
         goto erret;
      }
      ptr -> db_url = av.ptrvalue;
      atp = AsnReadId(aip,amp, atp);
   }
   if (atp == GBXREF_id_url) {
      if ( AsnReadVal(aip, atp, &av) <= 0) {
         goto erret;
      }
      ptr -> id_url = av.ptrvalue;
      atp = AsnReadId(aip,amp, atp);
   }
...
GBXrefAsnWrite(GBXrefPtr ptr, AsnIoPtr aip, AsnTypePtr orig)
{
   ...
   if (ptr -> db_url != NULL) {
      av.ptrvalue = ptr -> db_url;
      retval = AsnWrite(aip, GBXREF_db_url,  &av);
   }
   if (ptr -> id_url != NULL) {
      av.ptrvalue = ptr -> id_url;
      retval = AsnWrite(aip, GBXREF_id_url,  &av);
   }
   if (! AsnCloseStruct(aip, atp, (Pointer)ptr)) {
      goto erret;
   }
