...
#define REGISTER_MAKE_GROUPS_OF_100 ObjMgrProcLoadEx (OMPROC_FILTER,"Split Into Groups of 100 Bioseqs","SplitIntoGroupsOf100Bioseqs",0,0,0,0,NULL,MakeGroupsOf200,PROC_PRIORITY_DEFAULT, "Indexer")
...
#define REGISTER_DETACH ObjMgrProcLoadEx (OMPROC_FILTER,"Detach After Bioseq","DetachBioseq",OBJ_SEQFEAT,0,OBJ_SEQFEAT,0,NULL,DetachBioseq,PROC_PRIORITY_DEFAULT, "Misc")
...
#define REGISTER_FEAT_INTERVALS_TO_DELTA ObjMgrProcLoadEx (OMPROC_FILTER, "Feature Intervals to Delta Sequence", "FeatToDelta", 0,0,0,0,NULL, FeatToDeltaSeq, PROC_PRIORITY_DEFAULT, "Indexer")

#if defined(OS_UNIX) || defined(OS_MSWIN) 
#define REGISTER_CORRECTRNASTRANDSMART ObjMgrProcLoadEx (OMPROC_FILTER, "Correct RNA Strand Use SMART BLAST Results","CorrectRNAStrandSMART",0,0,0,0,NULL,CorrectRNAStrandednessUseSmart,PROC_PRIORITY_DEFAULT, "Analysis")
#define REGISTER_CORRECTRNASTRAND ObjMgrProcLoadEx (OMPROC_FILTER, "Correct RNA Strand Use Local Database Search","CorrectRNAStrandLocalDatabase",0,0,0,0,NULL,CorrectRNAStrandedness,PROC_PRIORITY_DEFAULT, "Analysis")
#endif
...

{
  ...
  if (GetInputFileName (path, sizeof (path), NULL, "TEXT")) {
    fp = FileOpen (path, "r");
    if (fp != NULL) {
      sequence_info = GetAlignmentOptions (&moltype, NULL);
...
static void DenseSegSwitchSeqPos (DenseSegPtr dsp)
{
  ...
  /* switch the IDs */
  sip = dsp->ids;
  sip_next = dsp->ids->next;
  sip_next->next = sip;
  sip->next = NULL;
  dsp->ids = sip_next;

  /* switch the starts and strands*/
  for (i = 0; i < dsp->numseg; i++) {
    tmp_start = dsp->starts[(i * dsp->dim)];
    dsp->starts[(i * dsp->dim)] = dsp->starts[(i * dsp->dim) + 1];
    dsp->starts[(i * dsp->dim) + 1] = tmp_start;
  }

  if (dsp->strands != NULL) {
    for (i = 0; i < dsp->numseg; i++) {
      tmp_strand = dsp->strands[(i * dsp->dim)];
      dsp->strands[(i * dsp->dim)] = dsp->strands[(i * dsp->dim) + 1];
      dsp->strands[(i * dsp->dim) + 1] = tmp_strand;
    }
  }
}


static BioseqPtr FindBioseqForSeqHist (SeqAlignPtr salp)
{
  SeqIdPtr  sip;
  BioseqPtr bsp;
  DenseSegPtr dsp;
  ...
  bsp = BioseqFind (sip);
  if (bsp == NULL) {
    /* if we find the bioseq here, need to switch the positions */
    sip = dsp->ids;
    bsp = BioseqFind (sip);
    if (bsp != NULL) {
      DenseSegSwitchSeqPos (dsp);
    }
  }
  return bsp;
}
...
static SeqIdPtr CorrectIDsForSeqHistAlign (TAlignmentFilePtr afp, SeqEntryPtr sep)
{
  SeqIdPtr sip;
  Boolean  need_switch = FALSE;
  char *tmp;

  if (afp == NULL || afp->num_sequences != 2) {
    return NULL;
  }
  if (StringNICmp (afp->ids[1], "acc", 3) == 0) {
    /* already have far pointer in place */
    sip = CreateSeqIdFromText (afp->ids[0], sep);
  } else if (StringNICmp (afp->ids[0], "acc", 3) == 0) {
    need_switch = TRUE;
    sip = CreateSeqIdFromText (afp->ids[1], sep);
  } else {
    /* first position should be primary */
    sip = CreateSeqIdFromText (afp->ids[0], sep);
    if (sip == NULL) {
      sip = CreateSeqIdFromText (afp->ids[1], sep);
      if (sip == NULL) {
        /* can't find match in record */
      } else {
        /* second position is primary, put acc on first position */
        need_switch = TRUE;
        tmp = malloc (StringLen (afp->ids[0]) + 4);
        sprintf (tmp, "acc%s", afp->ids[0]);
        free (afp->ids[0]);
        afp->ids[0] = tmp;
      }
    } else {
      tmp = malloc (StringLen (afp->ids[1]) + 4);
      sprintf (tmp, "acc%s", afp->ids[1]);
      free (afp->ids[1]);
      afp->ids[1] = tmp;
    }
  }
  ...
  if (sip != NULL) {
    if (need_switch) {
      /* note - not bothering to switch deflines and organism names because
       * we don't need them for the seq-hist assembly
       */
      tmp = afp->ids[0];
      afp->ids[0] = afp->ids[1];
      afp->ids[1] = tmp;
      ...
      tmp = afp->sequences[0];
      afp->sequences[0] = afp->sequences[1];
      afp->sequences[1] = tmp;
    }
  }
  return sip;  
...
extern void ImportAlignmentForSeqHistInterval (IteM i)
{
  ...
  if (GetInputFileName (path, sizeof (path), NULL, "TEXT")) {
    ...
    if (fp != NULL) {
      ...
      if (afp == NULL) {
        ProduceAlignmentNotes (afp, error_list);
      } else if (afp->num_sequences != 2) {
        Message (MSG_ERROR, "Must import alignment containing just the primary and TPA sequences");
      } else if ((sip = CorrectIDsForSeqHistAlign (afp, sep)) == NULL
                 || (bsp = BioseqFind (sip)) == NULL) {
        Message (MSG_ERROR, "Unable to locate TPA sequence in alignment");
      } else {
        sepnew = MakeSequinDataFromAlignmentEx (afp, moltype, TRUE);
        if (sepnew != NULL) {
          salpnew = (SeqAlignPtr) FindSeqAlignInSeqEntry (sepnew, OBJ_SEQALIGN);
          if (salpnew == NULL) {
            Message (MSG_ERROR, "No alignment created");
          } else if (salpnew->dim != 2) {
            Message (MSG_ERROR, "Must import alignment containing just the primary and TPA sequences");
          } else {
            sev = ErrSetMessageLevel (SEV_FATAL);
            ...
            */
            SeqEntrySetScope (scope);
            AlnMgr2IndexSingleChildSeqAlign(salpnew);
            UpdateOneSeqAlignFarPointer (salpnew, 1);
            ...
            if (ok) {
              /* find the bioseq that this interval should be added to */
              ...
              * indexed now.
              */
              AlnMgr2IndexSingleChildSeqAlign(salp_copy);

              /* add to Bioseq-hist assembly */
              if (bsp->hist == NULL) {
                bsp->hist = SeqHistNew ();
              }
              salp_copy->next = bsp->hist->assembly;
              bsp->hist->assembly = salp_copy;
...
/* This function finds the zero-based index of the single sequence in an alignment
 * that extends past the left side of an alignment (like a sore thumb).
 */
static Int4 FindAlignmentLeftThumb (SeqAlignPtr salp)
{
  DenseSegPtr dsp;
  Int4        k;
  Int4        sore_thumb = -1; /* because the sequence in question sticks out... */
  BioseqPtr   bsp;
  SeqIdPtr    sip;
  ...
  /* we need to examine the alignment, to see if there is a single sequence 
   * that extends before the beginning of the alignment or past the end of
   * the alignment, so that we can insert additional segments with gaps for
   * all other sequences.
   */
  
  dsp = (DenseSegPtr) salp->segs;
  
  /* check left end of alignment */
  for (k = 0, sip = dsp->ids; k < dsp->dim; k++, sip = sip->next)
  {
    if (dsp->strands [k] == Seq_strand_minus)
    {
      bsp = BioseqFind (sip);
      if (bsp != NULL 
          && dsp->starts [k] + dsp->lens [0] < bsp->length 
          && dsp->starts [k] > -1)
      {
        if (sore_thumb != -1)
        {
          /* can only do this when only one sequence extends past the end */
          return -1;
        }
        else
        {
...
/* This function finds the zero-based index of the single sequence in an alignment
 * that extends past the right side of an alignment (like a sore thumb).
 */
static Int4 FindAlignmentRightThumb (SeqAlignPtr salp)
{
  DenseSegPtr dsp;
  Int4        k, start_index;
  Int4        sore_thumb = -1; /* because the sequence in question sticks out... */
  ...
  /* check right end of alignment */
  for (k = 0, sip = dsp->ids; k < dsp->dim; k++, sip = sip->next)
  {
    start_index = (dsp->dim * (dsp->numseg - 1)) + k ;
    if (dsp->strands [start_index] == Seq_strand_minus)
    {
      if (dsp->starts [start_index] > 0)
      {
        if (sore_thumb != -1)
        {
          /* can only do this when only one sequence extends past the end */
    ...
    else
    {
      bsp = BioseqFind (sip);
      if (bsp != NULL 
          && dsp->starts [start_index] > -1
          && dsp->starts [start_index] + dsp->lens [dsp->numseg - 1] < bsp->length)
      {
        if (sore_thumb != -1)
        {
          /* can only do this when only one sequence extends past the end */
...
/* This function looks for a single sequence extending past the left side of the 
 * alignment and/or a single sequence extending past the right side of the alignment
 * and extends the alignment to cover these sequences, with gaps in the alignment for
 * all of the other sequences.
 * The function is unable to extend the alignment if more than one sequence extends
 * past the end of the alignment on that side.
 */
static void FixAlignmentEndStubs (SeqAlignPtr salp)
{
  DenseSegPtr dsp, dsp_new;
  Int4        k;
  Int4        left_thumb, right_thumb; /* because the sequence in question sticks out... */
  BioseqPtr   bsp;
  Int4        extra_segs = 0, seg_offset = 0;
  SeqIdPtr    sip;
  Int4        new_index, old_index;
  Int4        thumb_len;
  ...
  left_thumb = FindAlignmentLeftThumb (salp);
  right_thumb = FindAlignmentRightThumb (salp);
  
  if (left_thumb == -1 && right_thumb == 1)
  {
    return;
  }
  ...
  /* insert sequence for the thumb and gap for all of the other sequences at the
   * beginning of the alignment or end of the alignment.
   */
  ...
  dsp_new->strands = (Uint1Ptr) MemNew (dsp_new->dim * dsp_new->numseg * sizeof(Int4));
  dsp_new->ids = dsp->ids;
  dsp->ids = NULL;
  
  if (left_thumb != -1)
  {
    for (k = 0, sip = dsp_new->ids; k < dsp_new->dim; k++, sip = sip->next)
    {
      if (k == left_thumb)
      {
        if (dsp->strands [k] == Seq_strand_minus)
        {
          ...
          if (bsp == NULL)
          {
            dsp->ids = dsp_new->ids;
            dsp_new->ids = NULL;
            DenseSegFree (dsp_new);
            return;
          }
          thumb_len = bsp->length - dsp->starts [k] - dsp->lens [0];
          dsp_new->starts [k] = bsp->length - thumb_len;
          dsp_new->lens [0] = thumb_len;
          dsp_new->strands [k] = Seq_strand_minus;
        }
        else
        {
          dsp_new->starts [k] = 0;
          dsp_new->lens [0] = dsp->starts [k];
          dsp_new->strands [k] = Seq_strand_plus;
        }
      }
      else
      {
        dsp_new->starts [k] = -1;
        /* keep strand consistent with first segment */
        dsp_new->strands [k] = dsp->strands [k];
      }
    }
    seg_offset ++;
  }
  
  /* copy middle alignment starts and strands */
  for (k = 0; k < dsp->dim * dsp->numseg; k++)
  {
    dsp_new->starts [k + (dsp->dim * seg_offset)] = dsp->starts [k];
    dsp_new->strands [k + (dsp->dim * seg_offset)] = dsp->strands [k];
  }
  /* copy middle alignment lens */
  for (k = 0; k < dsp->numseg; k++)
  {
    dsp_new->lens [k + seg_offset] = dsp->lens [k];
  }
  
  /* add final segment */
  if (right_thumb != -1)
  {
    for (k = 0, sip = dsp_new->ids; k < dsp->dim; k++, sip = sip->next)
    {
      new_index = dsp_new->dim * (dsp_new->numseg - 1) + k;
      old_index = dsp->dim * (dsp->numseg - 1) + k;
      
      if (k == right_thumb)
      {
        if (dsp->strands [old_index] == Seq_strand_minus)
        {
          dsp_new->starts [new_index] = 0;
          dsp_new->strands [new_index] = Seq_strand_minus;
          dsp_new->lens [dsp_new->numseg - 1] = dsp->starts [old_index];
        }
        else
        {
          bsp = BioseqFind (sip);
          thumb_len = bsp->length - dsp->starts [old_index] - dsp->lens [dsp->numseg - 1];
          dsp_new->starts [new_index] = bsp->length - thumb_len;
          dsp_new->lens [dsp_new->numseg - 1] = thumb_len;
          dsp_new->strands [new_index] = Seq_strand_plus;
        }
      }
      else
      {
        dsp_new->starts [new_index] = -1;
        /* keep strands consistent */
        dsp_new->strands [new_index] = dsp->strands [old_index];
      }
    }
  }
  
  /* free the old alignment */
  dsp = DenseSegFree(dsp);
  
  /* replace it with the new alignment */
  salp->segs = (Pointer)(dsp_new);
  
  /* reindex the alignment */
...
extern Int2 AddSeqAlignForSeqEntry (SeqEntryPtr sep, Uint2 entityID, Boolean choose_master, Boolean use_new_blast)
{
  ...
  if (sep == NULL || sep->data.ptrvalue == NULL || !IS_Bioseq_set (sep)) return OM_MSG_RET_ERROR;
  ...
  for (vnp = seg_aln_list; vnp != NULL; vnp = vnp->next)
  {
    ...
    CreateOneAlignment (sbp, entityID, bssp, use_new_blast, fp, &errors_in_log);
  }
...

{
  ...
  switch (ompcp->input_itemtype) {
    case OBJ_BIOSEQ :
      Message (MSG_ERROR, "Must select BioseqSet to create alignment");
      return OM_MSG_RET_ERROR;
      break;
    case OBJ_BIOSEQSET :
      bssp = (BioseqSetPtr) ompcp->input_data;
  ...
  return AddSeqAlignForSeqEntry (sep, ompcp->input_entityID, choose_master, use_new_blast);
}
...
static ValNodePtr ListOrfs (BioseqPtr bsp, Boolean altstart, Int4 min_len, Boolean allow_partial);
...
static void AltProc(GrouP g)
{
	...
	ovp->orfs =  ListOrfs(ovp->bsp, ovp->alt_start, ovp->len, ovp->allow_partial);
	SortOrfs(ovp);
	PopulateOrfList (ovp);
...
static void ShowPartialOrfs (ButtoN b)
{
	OrfViewFormPtr ovp;

	ovp = (OrfViewFormPtr) GetObjectExtra(b);
  ...
  ovp->allow_partial = GetStatus (ovp->show_partial_btn);
...
typedef struct orfdata {
  ...
  ValNodePtr orf_list;
  SeqIdPtr   sip;
  Boolean    allow_partial_orf;
  Boolean    partial_other_end[6];
...

{
  ...
  Boolean     partial_this_end = FALSE;
  ...
  if (strand == Seq_strand_plus) {
    ...
    idx = frame;
    if (position == 0 && (atgStart || (altStart && odp->altstart)))
    {
      /* not partial at 5' end */
      odp->partial_other_end[idx] = FALSE;
    }

    if (odp->inorf [idx]) {
      if (!orfStop) {
        /* treat the end of the sequence like a stop codon */  
        if (odp->allow_partial_orf && TreatLikeStop(frame, position, strand, odp->bioseq_len)) {
          position += 3;
          (odp->curlen[idx])++;
          orfStop = TRUE;
          partial_this_end = TRUE;
        }
      }
      if (orfStop) {
        odp->inorf [idx] = FALSE;
        if (odp->curlen[idx] >= odp->min_len) {
          slp = SeqLocIntNew (odp->currstart [idx] + idx, 
                              MIN (position + 2, (Int4) odp->bioseq_len - 1),
                              strand, odp->sip);
          SetSeqLocPartial (slp, odp->partial_other_end[idx], partial_this_end);
    ...
    if (!orfStop && odp->allow_partial_orf) {
    ...
    } else if (atgStart || (altStart && odp->altstart) || (odp->allow_partial_orf && position >= odp->bioseq_len - 5)) {
      (odp->sublen [idx])++;
      odp->curlen [idx] = odp->sublen [idx];
      if (odp->allow_partial_orf && position >= odp->bioseq_len - 5)
      {
        /* also include partial codon at partial end */
        (odp->curlen[idx]) ++;
        if (!atgStart && (!altStart || !odp->altstart))
        {
          partial_this_end = TRUE;
        }
      }
      if (odp->curlen[idx] >= odp->min_len) {
        slp = SeqLocIntNew (MAX ((Int4) odp->currstart [idx] + idx - 3, (Int4) 0), 
                            MIN (odp->currstart [idx] + idx - 3 + (odp->curlen [idx]) * 3 + 2, odp->bioseq_len - 1),
                            Seq_strand_minus, odp->sip);
        SetSeqLocPartial (slp, partial_this_end, odp->partial_other_end[idx]);
...

{
  ...
  for (i = 0; i < 6; i++) {
    ...
    od.inorf [i] = FALSE;
    od.lastvnp [i] = NULL;
    od.partial_other_end [i] = allow_partial;
  }

  if (allow_partial) {
    /* allow 5' partial ORFs */
    for (i = 0; i < 3; i++) {
      od.inorf[i] = TRUE;
      od.curlen [i] = 1;
      od.currstart [i] = 0 - i;
  ...
  od.allow_partial_orf = allow_partial;
...
extern void LaunchOrfViewer (BioseqPtr bsp, Uint2 entityID, Uint4 itemID, Boolean standAlone)
{
	...
	ovp->len = 10;
	ovp->alt_start = FALSE;
  ovp->allow_partial = TRUE;
#if 1
    ovp->orfs = ListOrfs (bsp, ovp->alt_start, ovp->len, ovp->allow_partial);
#else	
  ...
  ovp->show_partial_btn = CheckBox (g, "Show Partial ORFs", ShowPartialOrfs);
  SetObjectExtra (ovp->show_partial_btn, ovp, NULL);
  SetStatus (ovp->show_partial_btn, TRUE);
...

{
  ...
  switch (gcp->thistype) {
    ...
    case OBJ_SEQFEAT :
      sfp = (SeqFeatPtr) gcp->thisitem;
      if (sfp != NULL ) {
        slp = (SeqLocPtr) sfp->location;
        if (slp != NULL) {
          sip = SeqLocId (slp);
          CacheByAccn (sip, path);
        }
        slp = (SeqLocPtr) sfp->product;
        if (slp != NULL) {
          sip = SeqLocId (slp);
          CacheByAccn (sip, path);
        }
...
{
  ProcessMultipleBioseqFunctions ((OMProcControlPtr) data, BioseqRevComp, TRUE, TRUE);
  return OM_MSG_RET_DONE;
}
...
{
  ProcessMultipleBioseqFunctions ((OMProcControlPtr) data, BioseqRevComp, FALSE, TRUE);
  return OM_MSG_RET_DONE;
}
...
{
  ProcessMultipleBioseqFunctions ((OMProcControlPtr) data, BioseqReverse, FALSE, TRUE);
  return OM_MSG_RET_DONE;
}
...
{
  ProcessMultipleBioseqFunctions ((OMProcControlPtr) data, BioseqComplement, FALSE, FALSE);
  return OM_MSG_RET_DONE;
}
...

{
  ...
  if (GetAppParam ("SEQUIN", "SETTINGS", "ALLOWDETACH",
                    NULL, str, sizeof (str)) &&
  ...
  if (indexerVersion) {
    ...
    REGISTER_CORRECTRNASTRANDSMART;
  }
#endif
...

{
  ...
  has_segsets = SeqEntryHasSegSets (sep);
  if (has_segsets) 
  {
 ...
 CharPtr PNTR serotype)
{
  ...
  if (first_paren == NULL) return FALSE;
  cp = first_paren + 1;
  while (*cp != ')' && *cp != '(' && *cp != 0)
  {
    cp++;
 ...
 ValNodePtr PNTR  affected_pubs)
{
           ...
           && field_to_set <= PUB_FIELD_CONSORTIUM)
  {
    ...
    {
      if (field_to_set == PUB_FIELD_CONSORTIUM) 
      {
        for (names = alp->names; names != NULL; names = names->next) 
        {
          ap = (AuthorPtr) names->data.ptrvalue;
          if (ap != NULL && ap->name->choice == 5 && DoesOneAuthorMatchString (ap, p)) 
          {
            SetAuthorString (ap, field_to_set, str);
            changed = TRUE;
          }
        }
      } 
...
{
  if (pub == NULL || epfp == NULL) return FALSE;
...
static Pointer DoubleListDialogToListPair (DialoG d)
{
  ...
  list_tmp = NULL;
  for (vnp = dlp->list1; vnp != NULL; vnp = vnp->next) {
    ValNodeAddPointer (&list_tmp, vnp->choice, vnp->data.ptrvalue);
  }
  ValNodeAddPointer (&list_pair, 0, list_tmp);
  
  list_tmp = NULL;
  for (vnp = dlp->list2; vnp != NULL; vnp = vnp->next) {
    ValNodeAddPointer (&list_tmp, vnp->choice, vnp->data.ptrvalue);
  }
  ValNodeAddPointer (&list_pair, 0, list_tmp);
  return list_pair;
...
static void CleanupDoubleListDialog (GraphiC g, VoidPtr data)
{
  DoubleListPtr dlp;

  dlp = (DoubleListPtr) data;
  if (dlp != NULL) 
  {
    dlp->list1 = ValNodeFree (dlp->list1);
    dlp->list2 = ValNodeFree (dlp->list2);
  }
 ...
 Pointer                  change_userdata)
{
  ...
  SetObjectExtra (p, dlp, CleanupDoubleListDialog);
...
static void EnableAdjustListPairAccept (Pointer data)
{
  ...
  for (vnp = list_pair; vnp != NULL; vnp = vnp->next) {
...

{
  ...
  /* clean up list pair returned by dialog */
  for (vnp = list_pair; vnp != NULL; vnp = vnp->next) {
    vnp->data.ptrvalue = ValNodeFree (vnp->data.ptrvalue);
  }
  list_pair = ValNodeFree (list_pair);
...
static void ConvertOneBioseqToDelta (BioseqPtr bsp)
{
  CharPtr       bases;
  ValNodePtr    seq_ext;
  SeqLitPtr     slp;
  ...
  if (slp != NULL) 
  {
    ...
    AddBasesToByteStore ((ByteStorePtr) slp->seq_data, bases);
  }
...
static void ListBioseqSetRawSequences (BioseqSetPtr bssp, ValNodePtr PNTR vnpp)
{
  SeqEntryPtr sep;
  BioseqPtr   bsp;
  Char        id_str [128];
  
  if (bssp == NULL || bssp->_class == BioseqseqSet_class_parts || vnpp == NULL)
  {
    return;
  }
  ...
  for (sep = bssp->seq_set; sep != NULL; sep = sep->next)
  {
    if (IS_Bioseq_set (sep))
    {
      ListBioseqSetRawSequences (sep->data.ptrvalue, vnpp);
    }
    else if (IS_Bioseq (sep))
    {
      bsp = (BioseqPtr) (sep->data.ptrvalue);
      ...
      {
        SeqIdWrite (SeqIdFindBest (bsp->id, SEQID_GENBANK), id_str, PRINTID_REPORT, sizeof (id_str) - 1);
        ValNodeAddPointer (vnpp, 0, StringSave (id_str));
      }
    }
  }
}

static Boolean ListRawSequencesInAlignments (GatherContextPtr gcp)

{
  ...
  /* don't process parts of a segmented set */
  if (gcp->parenttype == OBJ_BIOSEQSET
      && (bssp = (BioseqSetPtr) gcp->parentitem) != NULL
      && bssp->_class == BioseqseqSet_class_parts)
  {
    return TRUE;
  }
  ...
  if (gcp->thistype == OBJ_BIOSEQ) {
    bsp = (BioseqPtr) gcp->thisitem;
    if (bsp->repr == Seq_repr_raw && IsBioseqInAnyAlignment (bsp, gcp->entityID))
    {
      SeqIdWrite (SeqIdFindBest (bsp->id, SEQID_GENBANK), id_str, PRINTID_REPORT, sizeof (id_str) - 1);
      ValNodeAddPointer (vnpp, 0, StringSave (id_str));
    }
    ...
    bssp = (BioseqSetPtr) gcp->thisitem;
    ListBioseqSetRawSequences (bssp, vnpp);
  } else return TRUE;
  
  return TRUE;
}

static void ConvertBioseqSetContentsToDelta (BioseqSetPtr bssp)
{
  SeqEntryPtr sep;
  ...
  for (sep = bssp->seq_set; sep != NULL; sep = sep->next)
  {
    if (IS_Bioseq_set (sep))
    {
      ConvertBioseqSetContentsToDelta (sep->data.ptrvalue);
    }
    else if (IS_Bioseq (sep))
    {
      ConvertOneBioseqToDelta (sep->data.ptrvalue);
    }
  }
}

static Boolean ConvertBioseqOrBioseqSetContentsToDelta (GatherContextPtr gcp)

{
  BioseqPtr        bsp;
  BioseqSetPtr     bssp;
  ValNodePtr PNTR  vnpp;
  ValNodePtr       entity_vnp;
  ...
  if (gcp->thistype == OBJ_BIOSEQ) {
    bsp = (BioseqPtr) gcp->thisitem;
    ConvertOneBioseqToDelta (bsp);
  } else if (gcp->thistype == OBJ_BIOSEQSET) {
    bssp = (BioseqSetPtr) gcp->thisitem;
    ConvertBioseqSetContentsToDelta (bssp);
  } else return TRUE;
  
  for (entity_vnp = *vnpp;
       entity_vnp != NULL && entity_vnp->data.intvalue != gcp->entityID;
       entity_vnp = entity_vnp->next)
  {
  }
  if (entity_vnp == NULL)
  {
    ValNodeAddInt (vnpp, 0, gcp->entityID);
  }
  return TRUE;
}
...

{
  OMProcControlPtr  ompcp;
  SelStructPtr      sel;
  ValNodePtr        entityID_list = NULL, entity_vnp, raw_in_aln = NULL;
  CharPtr           msg = NULL;
  MsgAnswer         ans;
  ...
  for (sel = ObjMgrGetSelected (); sel != NULL; sel = sel->next) {
    GatherItem (sel->entityID, sel->itemID, sel->itemtype,
                (Pointer) &raw_in_aln, ListRawSequencesInAlignments);
  }
  if (raw_in_aln != NULL)
  {
  	msg = CreateListMessage ("Sequence", 
  ...
  for (sel = ObjMgrGetSelected (); sel != NULL; sel = sel->next) {
    GatherItem (sel->entityID, sel->itemID, sel->itemtype,
                (Pointer) &entityID_list, ConvertBioseqOrBioseqSetContentsToDelta);
  }
  if (entityID_list == NULL) return OM_MSG_RET_DONE;
  
  for (entity_vnp = entityID_list;
       entity_vnp != NULL;
       entity_vnp = entity_vnp->next)
  {
    ObjMgrSetDirtyFlag (entity_vnp->data.intvalue, TRUE);
    ObjMgrSendMsg (OM_MSG_UPDATE, entity_vnp->data.intvalue, 0, 0);
  }
  Update ();
  return OM_MSG_RET_DONE;
}
...

{
  ...
  if (SeqMgrGetDesiredFeature (ompcp->input_entityID, NULL, 0, 0, sfp, &context) != sfp) return OM_MSG_RET_ERROR;
  bsp = BioseqFindFromSeqLoc (sfp->location);
  if (bsp == NULL) return OM_MSG_RET_ERROR;
  sip = SeqIdFindBest (bsp->id, 0);
  if (sip == NULL) return OM_MSG_RET_ERROR;

  if (! GetOutputFileName (path, sizeof (path), NULL)) return OM_MSG_RET_ERROR;
  fp = FileOpen (path, "w");
  if (fp == NULL) return OM_MSG_RET_ERROR;
  ...
  stop = *ivals;
  ivals++;
  sint.from = MIN (start, stop);
  sint.to = MAX (start, stop);
  sint.strand = context.strand;
  sint.id = sip;
  vn.choice = SEQLOC_INT;
  vn.data.ptrvalue = (Pointer) &sint;
  SeqLocFastaStream (&vn, fp, 0, 60, 0, 0);
  last = stop;
  for (i = 1; i < context.numivals; i++) {
    start = *ivals;
    ivals++;
    stop = *ivals;
    ivals++;
...
static void RemoveUnpublishedPubFeatCallback (SeqFeatPtr sfp, Pointer userdata)
{
  PubdescPtr pdp;

  if (sfp == NULL || sfp->data.choice != SEQFEAT_PUB || sfp->data.value.ptrvalue == NULL)
  {
    return;
  }
  pdp = (PubdescPtr) sfp->data.value.ptrvalue;
  if (GetPubStatus (pdp->pub) == PUB_STAT_UNPUBLISHED)
  {
    sfp->idx.deleteme = TRUE;
  }
}

static void RemoveUnpublishedPubDescCallback (SeqDescPtr sdp, Pointer userdata)
{
  PubdescPtr    pdp;
  ObjValNodePtr ovp;
...
extern void RemoveUnpublishedPublications (IteM i)
{
  ...
  VisitDescriptorsInSep (sep, NULL, RemoveUnpublishedPubDescCallback);
  VisitFeaturesInSep (sep, NULL, RemoveUnpublishedPubFeatCallback);
...
static void RemovePublishedPubFeatCallback (SeqFeatPtr sfp, Pointer userdata)
{
  PubdescPtr pdp;
  ...
  pdp = (PubdescPtr) sfp->data.value.ptrvalue;
  if (GetPubStatus (pdp->pub) == PUB_STAT_PUBLISHED)
  {
    sfp->idx.deleteme = TRUE;
  }
}

static void RemovePublishedPubDescCallback (SeqDescPtr sdp, Pointer userdata)
{
  PubdescPtr    pdp;
  ObjValNodePtr ovp;
...
extern void RemovePublishedPublications (IteM i)
{
  ...
  VisitDescriptorsInSep (sep, NULL, RemovePublishedPubDescCallback);
  VisitFeaturesInSep (sep, NULL, RemovePublishedPubFeatCallback);
