...
*/
   ...
   for(i=sptree.nspecies; i<tree.nnode; i++) {
      if(nodes[i].nodeStr == NULL) 
         continue;
      if(nodes[i].fossil) {  /* fossila specified using <, >, already processed.  */
...
/* This calculates the likelihood using the normal approxiamtion (Thorne et al. 
   ...
   the branch length to the fist son of the root.  
...
*/
   ...
   for(j=0; j<tree.nnode; j++) {
      ...
      ib = nodes[j].ibranch;
      if(j==son1)
         z[ib] = nodes[son1].branch + nodes[son2].branch - data.blMLE[locus][ib];
      else
         ...
         for (iopt=0; iopt<nopt; iopt++) {
            if (strncmp(opt, optstr[iopt], 8)==0)  {
               ...
               switch (iopt) {
                  ...
                  case ( 8): sscanf(pline+1, "%d %s", &mcmc.usedata, com.inBVf);
